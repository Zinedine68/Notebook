...
void parabolicVelocityFvPatchVectorField2Dpf::updateCoeffs()
{
	...
	scalar R(0.004);
	scalar U_tau(0.0218); // to make the bulk velocity to 0.3
	...
	scalarField yPlus = y1 * U_tau / 1.0e-6;
	...
	//vectorField::operator=(n_*maxValue_*(1.0 - coord_pow2));

	// after calculating the mean velocity profile
	//scalar apt(0.4);
	//scalar apt(0.);
	//scalarField apt_array=apt*(1.0 - sqr(sqrt(coord_pow2)-1.0));
	scalarField envelope_Rz = U_tau * envelopeUzRMS(yPlus);
	scalarField envelope_Rr = U_tau * envelopeUrRMS(yPlus);
    ...
    //
        ...
        forAll(patchField, facei)
        {
			...
			if (yPlus[facei] < 5)
			{
				...
				patchField[facei] = n_ * U_tau * viscousLayer(yPlus[facei]);
			}
			else if ( 5 <= yPlus[facei] && yPlus[facei] < 30)
			{
				patchField[facei] = n_ * U_tau * bufferLayer(yPlus[facei]);
			}
			else if (yPlus[facei] >= 30) // no upper limit here, there should be one
			{
				patchField[facei] = n_ * U_tau * logLayer(yPlus[facei]);
			}
			else
			{
